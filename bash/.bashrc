...
# pnpm
export PNPM_HOME="/home/willie/.local/share/pnpm"
case ":$PATH:" in
  *":$PNPM_HOME:"*) ;;
...
# Source custom scripts if available
if [ -f ~/.bash_aliases ]; then
    . ~/.bash_aliases
fi

## ALIASES ##
# adding text to clipboard, uses stdin
alias cb='clip.exe'

# writing from clipboard to stdout
alias cbp='powershell.exe Get-Clipboard | sed "s/\r$//"'

alias gs='git status'
alias gtree='git log --graph --oneline --decorate --all'
alias gb='git branch | grep \* | sed -e "s/* //"'
alias gauthor='git log --pretty=format:"(%ad) %an - %s" --date=iso'
alias vim='nvim'
alias vi='nvim'
